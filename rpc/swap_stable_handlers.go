...
func (s *Server) handleStableRequestSwapApproval(w http.ResponseWriter, r *http.Request, req *RPCRequest) {
	...
	quote, err := engine.Price(r.Context(), stablequote.QuoteRequest{Asset: asset, Amount: amount})
	if err != nil {
...
func (s *Server) handleStableSwapMint(w http.ResponseWriter, r *http.Request, req *RPCRequest) {
	...
	reservation, err := engine.Reserve(r.Context(), stablequote.ReserveRequest{QuoteID: quoteID, Account: account, AmountIn: payload.AmountIn})
	if err != nil {
		s.writeStableRPCError(w, req.ID, err)
		return
	}
...
func (s *Server) handleStableSwapBurn(w http.ResponseWriter, r *http.Request, req *RPCRequest) {
	...
	intent, err := engine.CashOut(r.Context(), stablequote.CashOutRequest{ReservationID: reservationID})
	if err != nil {
		s.writeStableRPCError(w, req.ID, err)
		return
	}
...
func (s *Server) stableEngineConfig() (stablequote.Engine, map[string]stablequote.Asset, stablequote.Limits, func() time.Time) {
	s.swapStableMu.RLock()
	defer s.swapStableMu.RUnlock()
	assets := s.swapStable.assets
	if assets == nil {
		assets = make(map[string]stablequote.Asset)
	}
...
func (s *Server) writeStableRPCError(w http.ResponseWriter, id interface{}, err error) {
	switch {
	case errors.Is(err, stablequote.ErrNotSupported):
		writeError(w, http.StatusNotFound, id, codeInvalidParams, err.Error(), nil)
	case errors.Is(err, stablequote.ErrQuoteNotFound):
		writeError(w, http.StatusNotFound, id, codeInvalidParams, err.Error(), nil)
	case errors.Is(err, stablequote.ErrQuoteExpired):
		writeError(w, http.StatusConflict, id, codeInvalidParams, err.Error(), nil)
	case errors.Is(err, stablequote.ErrReservationNotFound):
...
func quoteCrossAsset(engine stablequote.Engine, payAsset string, getAsset string, amountInWei string) (string, error) {
	if engine == nil {
		return "", stablequote.ErrPriceUnavailable
	}
	payRate, _, _, payOK := engine.CurrentPrice("USD", payAsset)
	getRate, _, _, getOK := engine.CurrentPrice("USD", getAsset)
