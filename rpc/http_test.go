...
func TestHandleSendTransactionParsesHexNumericFields(t *testing.T) {
	db := storage.NewMemDB()
...
func TestHandleSendTransactionRejectsOversizedRecipientBeforeSignatureRecovery(t *testing.T) {
	server := newTestServer(t, nil, nil, ServerConfig{})
	param, err := json.Marshal(map[string]interface{}{
		"chainId":  "0x4e4842",
		"type":     types.TxTypeTransfer,
		"nonce":    "0x1",
		"to":       bytes.Repeat([]byte{0x01}, 21),
		"value":    "0x1",
		"gasLimit": "0x5208",
		"gasPrice": "0x1",
	})
	...
	if resp.Error == nil || !strings.Contains(resp.Error.Message, "to length") {
		t.Fatalf("expected oversized recipient validation error, got %+v", resp.Error)
	}
}
