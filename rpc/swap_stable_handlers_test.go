...
func TestStableRPCHandlersFlow(t *testing.T) {
	...
	limits := stablequote.Limits{DailyCap: 1_000_000}
	asset := stablequote.Asset{
		Symbol:         "ZNHB",
		BasePair:       "ZNHB",
		QuotePair:      "USD",
	...
	srv.ConfigureStableEngine(engine, limits, []stablequote.Asset{asset}, func() time.Time { return base.Add(10 * time.Second) })
	...
	if quoteID != "q-1717787717000000000" {
		t.Fatalf("unexpected quoteId %s", quoteID)
	}
	if quoteResult["expiresAt"].(string) != "2024-06-07T19:16:17Z" {
	...
	if reserveResult["expiresAt"].(string) != "2024-06-07T19:16:17Z" {
	...
	if burnResult["intentId"].(string) != "i-1717787718000000000" {
		t.Fatalf("unexpected intentId %v", burnResult["intentId"])
	}
	if burnResult["createdAt"].(string) != "2024-06-07T19:15:18Z" {
...
func newFakeStableEngine(clock func() time.Time, quoteTTL time.Duration, assets ...string) *fakeStableEngine {
	...
	return &fakeStableEngine{
		clock:        clock,
		quoteTTL:     quoteTTL,
		assets:       assetSet,
		quotes:       make(map[string]time.Time),
		reservations: make(map[string]stablequote.Reservation),
	}
}

func (f *fakeStableEngine) Price(_ context.Context, req stablequote.QuoteRequest) (stablequote.QuoteResponse, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if !f.assets[req.Asset] {
		return stablequote.QuoteResponse{}, stablequote.ErrNotSupported
	}
	now := f.clock()
	id := fmt.Sprintf("q-%d", now.UnixNano())
	expiresAt := now.Add(f.quoteTTL)
	f.quotes[id] = expiresAt
	return stablequote.QuoteResponse{Quote: stablequote.Quote{ID: id, Asset: req.Asset, Price: 1_000_000, ExpiresAt: expiresAt}}, nil
}

func (f *fakeStableEngine) Reserve(_ context.Context, req stablequote.ReserveRequest) (stablequote.ReserveResponse, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	expiresAt, ok := f.quotes[req.QuoteID]
	if !ok {
		return stablequote.ReserveResponse{}, stablequote.ErrQuoteNotFound
	}
	delete(f.quotes, req.QuoteID)
	amount := int64(req.AmountIn)
	reservation := stablequote.Reservation{QuoteID: req.QuoteID, AmountIn: amount, AmountOut: amount, ExpiresAt: expiresAt, Account: req.Account}
	f.reservations[req.QuoteID] = reservation
	return stablequote.ReserveResponse{Reservation: reservation}, nil
}

func (f *fakeStableEngine) CashOut(_ context.Context, req stablequote.CashOutRequest) (stablequote.CashOutResponse, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	reservation, ok := f.reservations[req.ReservationID]
	if !ok {
		return stablequote.CashOutResponse{}, stablequote.ErrReservationNotFound
	}
	delete(f.reservations, req.ReservationID)
	now := f.clock()
	id := fmt.Sprintf("i-%d", now.UnixNano())
	intent := stablequote.CashOutIntent{
		ID:            id,
		ReservationID: req.ReservationID,
		Amount:        float64(reservation.AmountOut),
		AmountUnits:   reservation.AmountOut,
		Account:       reservation.Account,
		CreatedAt:     now,
	}
	return stablequote.CashOutResponse{Intent: intent}, nil
}
...
func (f *fakeStableEngine) CurrentPrice(base, quote string) (float64, time.Time, bool, bool) {
	return 1.0, f.clock(), false, true
}

func newStableRPCTestEngine(t *testing.T, base time.Time) *fakeStableEngine {
	t.Helper()
	var counter int
	clock := func() time.Time {
