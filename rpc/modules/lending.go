...
func (m *LendingModule) applyComputedAPY(market *lending.Market) {
	...
	// Mirror native/lending/engine.go's real accrual math exactly (the
	// combine-then-cap-at-10000bps step right before it derives the supply
	// rate) -- this RPC-only display value must never drift from what the
	// engine actually deducts, or DepositApyBps overstates real yield by
	// the protocol fee share whenever protocolBps > 0.
	protocolBps := m.node.LendingProtocolFeeBps()
	...
	supplyAPY := model.SupplyAPY(market.TotalNHBBorrowed, market.TotalNHBSupplied, combinedBps)
