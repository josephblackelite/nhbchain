...
type potsoWeightParams struct {
	Epoch   *uint64 `json:"epoch,omitempty"`
	Address string  `json:"address"`
}

type potsoWeightResult struct {
	Epoch     uint64 `json:"epoch"`
	Address   string `json:"address"`
	WeightBps uint64 `json:"weightBps"`
}

// handlePotsoGetWeight answers "what voting power would native/governance's
// CastVote compute for this address right now" -- deliberately calling the
// SAME Node.PotsoWeight (SnapshotPotsoWeights-backed) lookup CastVote
// itself uses, not PotsoLeaderboard's PotsoMetricsGetSnapshot key. Unlike
// CastVote, a not-found address returns weightBps: 0 rather than an error,
// so a wallet can render "you have 0 voting power" before ever attempting
// a vote.
func (s *Server) handlePotsoGetWeight(w http.ResponseWriter, _ *http.Request, req *RPCRequest) {
	var params potsoWeightParams
	if len(req.Params) > 0 {
		if err := json.Unmarshal(req.Params[0], &params); err != nil {
			writeError(w, http.StatusBadRequest, req.ID, codeInvalidParams, "invalid parameters", err.Error())
			return
	...
	if trimmed == "" {
		writeError(w, http.StatusBadRequest, req.ID, codeInvalidParams, "address is required", nil)
		return
	}
	...
	actualEpoch, weightBps, err := s.node.PotsoWeight(epoch, addr)
	if err != nil {
		writeError(w, http.StatusInternalServerError, req.ID, codeServerError, "failed to load potso weight", err.Error())
		return
	}
	writeResult(w, req.ID, potsoWeightResult{Epoch: actualEpoch, Address: trimmed, WeightBps: weightBps})
}
