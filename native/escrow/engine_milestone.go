...
func (e *MilestoneEngine) hasOpenLegs(project *MilestoneProject) bool {
	if project == nil {
		return false
	}
	for _, leg := range project.Legs {
		...
		if leg.Status == MilestoneLegPending || leg.Status == MilestoneLegFunded {
			return true
		}
	}
	return false
