...
const (
	...
	// DefaultRedeemPerTxMinWei floors a single redemption at 5 NHB ($5).
	// Raised from "0" (2026-09-17) because redemption payouts carry a flat
	// fee deducted off-chain -- without a floor comfortably above that fee,
	// a customer could burn NHB on-chain (irreversible) for a redemption
	// whose off-chain payout would be zero or negative once the fee is
	// applied, since the burn and the fee/payout computation happen in two
	// separate, non-atomic steps (on-chain burn first, off-chain payout
	// second). $5 leaves a comfortable margin above that fee.
	DefaultRedeemPerTxMinWei = "5000000000000000000"
)
