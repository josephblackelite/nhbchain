...
// NHBPortalOracle fetches a manually-curated reference rate from a narrow,
// bearer-token-gated internal endpoint. Unlike CoinGeckoOracle or
// NowPaymentsOracle, this is not a discovered market price -- ZNHB has no
// real external listing (see docs/tokenomics/tokenomics.md) -- it is a
// considered human judgment entered through a separate admin system's own
// approval workflow. This is one input among several an off-chain buyback
// service can use; the exact upstream system and its endpoint/credential are
// deployment configuration outside the scope of this node's public source.
...
// NewNHBPortalOracle constructs the adapter. endpoint and token are
// deployment configuration for the upstream reference-rate source.
