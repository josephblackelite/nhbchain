...
func TestStableStoreDepositVoucherIdempotent(t *testing.T) {
	...
	inventory, err := store.GetSoftInventory(StableAssetUSDC)
	if err != nil {
...
func TestStableStoreCashOutLifecycleBurnAfterReceipt(t *testing.T) {
	...
	if recordedIntent.Status != CashOutStatusPending {
	...
	if settledIntent.Status != CashOutStatusSettled {
...
func TestStableStoreAbortCashOutIntent(t *testing.T) {
	backend := newMockStorage()
	store := NewStableStore(backend)
	store.SetClock(func() time.Time { return time.Unix(1700002000, 0) })

	deposit := &DepositVoucher{
		InvoiceID:    "inv-abort",
		Provider:     "nowpayments",
		StableAsset:  StableAssetUSDC,
		StableAmount: big.NewInt(4_000_000),
		NhbAmount:    big.NewInt(4_000_000_000_000_000_000),
		Account:      "nhb1abort",
	}
	if err := store.PutDepositVoucher(deposit); err != nil {
		t.Fatalf("put deposit: %v", err)
	...
	intent := &CashOutIntent{
		...
		StableAsset:  StableAssetUSDC,
		StableAmount: big.NewInt(1_500_000),
		NhbAmount:    big.NewInt(1_500_000_000_000_000_000),
	}
	if err := store.CreateCashOutIntent(intent); err != nil {
		t.Fatalf("create intent: %v", err)
	}

	if err := store.AbortCashOutIntent(intent.IntentID); err != nil {
		t.Fatalf("abort intent: %v", err)
	}

	recorded, ok, err := store.GetCashOutIntent(intent.IntentID)
	if err != nil || !ok {
		t.Fatalf("get aborted intent: %v ok=%v", err, ok)
	}
	if recorded.Status != CashOutStatusAborted {
		t.Fatalf("expected aborted status, got %s", recorded.Status)
	}
	lock, ok, err := store.GetEscrowLock(intent.IntentID)
	if err != nil || !ok {
		t.Fatalf("get escrow lock: %v ok=%v", err, ok)
	}
	if lock.Burned {
		t.Fatalf("expected abort to keep escrow unburned")
	}
	if receipt, ok, err := store.GetPayoutReceipt(intent.IntentID); err != nil || ok || receipt != nil {
		t.Fatalf("expected no payout receipt after abort, got receipt=%v ok=%v err=%v", receipt, ok, err)
	}
	if total := backend.supplies["NHB"]; total == nil || total.Cmp(deposit.NhbAmount) != 0 {
		t.Fatalf("expected supply unchanged after abort, got %v", total)
	}
	inventory, err := store.GetSoftInventory(StableAssetUSDC)
	if err != nil {
		t.Fatalf("get inventory: %v", err)
	}
	if inventory.Balance.Cmp(deposit.StableAmount) != 0 {
		t.Fatalf("expected inventory balance unchanged after abort, got %s want %s", inventory.Balance, deposit.StableAmount)
	}
	if err := store.RecordPayoutReceipt(&PayoutReceipt{
		ReceiptID:    "rcpt-after-abort",
		IntentID:     intent.IntentID,
		StableAsset:  intent.StableAsset,
		StableAmount: intent.StableAmount,
		NhbAmount:    intent.NhbAmount,
	}); err == nil {
		t.Fatalf("expected payout receipt to fail for aborted intent")
	}
}
