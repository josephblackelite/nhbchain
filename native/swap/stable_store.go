...
// AbortCashOutIntent cancels a pending intent without burning escrowed NHB.
func (s *StableStore) AbortCashOutIntent(intentID string) error {
	if s == nil {
		return fmt.Errorf("stable: store not initialised")
	}
	trimmedID := strings.TrimSpace(intentID)
	if trimmedID == "" {
		return fmt.Errorf("stable: intent id required")
	}
	intent, ok, err := s.GetCashOutIntent(trimmedID)
	if err != nil {
		return err
	}
	if !ok {
		return fmt.Errorf("stable: intent %s not found", trimmedID)
	}
	if intent.Status != CashOutStatusPending {
		return fmt.Errorf("stable: intent %s is not pending", trimmedID)
	}
	var lock storedEscrowLock
	ok, err = s.store.KVGet(stableEscrowLockKey(trimmedID), &lock)
	if err != nil {
		return err
	}
	if !ok {
		return fmt.Errorf("stable: escrow lock missing for %s", trimmedID)
	}
	if lock.Burned {
		return fmt.Errorf("stable: escrow already burned for %s", trimmedID)
	}
	intent.Status = CashOutStatusAborted
	intent.SettledAt = 0
	storedIntent := storedCashOutIntent{
		IntentID:     intent.IntentID,
		InvoiceID:    intent.InvoiceID,
	...
	return s.store.KVPut(stableCashOutIntentKey(trimmedID), storedIntent)
}
