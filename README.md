...
- **Function:** `ZNHB` has a hard genesis supply of exactly 1,000,000,000 tokens (the 8,000 above the originally-intended 1,000,000,000 is a documented reconciliation remainder from pre-existing, since-fixed mint-path bugs — see `docs/tokenomics/tokenomics.md`), split once at genesis into an 800,000,000-ZNHB **Sale Pool** and a 200,000,000-ZNHB **Reward Pool**. Nothing is ever minted to satisfy a purchase or a reward — both pools only ever move ZNHB that already exists.
...
> - `node.env`
>
> Use the template under [`deploy/env/`](./deploy/env) (`node.env.example`) as your starting point; see [`docs/deploy/one-shot-deploy.md`](./docs/deploy/one-shot-deploy.md) for the full required-config reference.
...
- **Audit phases:** [Overview](docs/audit/overview.md), [Static analysis](docs/audit/static-analysis.md), [Fuzzing](docs/audit/fuzzing.md), [End-to-end flows](docs/audit/e2e-flows.md), and [Documentation quality](docs/audit/docs-quality.md).
...
This codebase is released under the [Apache License, Version 2.0](./LICENSE). In brief:
you may use, modify, and distribute this code (including commercially), provided
you preserve the license and copyright notice and mark any files you change.
The license includes an explicit patent grant from contributors and does **not**
grant any rights to NHBCoin's trademarks or brand assets, which remain governed
by the Legal Notice above.
