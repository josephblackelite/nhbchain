...
// creator_tip is disabled server-side (410 Gone): it used to mutate
...
const RETIRED_MESSAGE =
  'creator_tip is disabled -- it mutated validator-local state outside the block pipeline, guaranteeing a consensus fork/halt on a 2-validator zero-quorum-slack chain; a signed-transaction replacement is pending.';
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json(
    { error: RETIRED_MESSAGE, retired: true, method: 'creator_tip' },
    { status: 410 },
  );
}
