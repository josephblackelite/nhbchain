...
func TestManualReviewRequiredForLargePayout(t *testing.T) {
	policies := []payoutd.Policy{{
		Asset:              "USDC",
		DailyCap:           big.NewInt(10_000_000_000),
		SoftInventory:      big.NewInt(10_000_000_000),
		Confirmations:      1,
		RequireReviewAbove: big.NewInt(500_000_000),
	}}
	enforcer, err := payoutd.NewPolicyEnforcer(policies)
	require.NoError(t, err)
	enforcer.SetInventory("USDC", big.NewInt(10_000_000_000))
	...
	require.ErrorIs(t, err, payoutd.ErrManualReviewRequired)

	err = processor.Process(context.Background(), payoutd.CashOutRequest{
		Intent:      intent,
		Destination: "0xrecipient",
		Approval: payoutd.ApprovalMetadata{
...
func TestVelocityRejectsTooManyPayouts(t *testing.T) {
	policies := []payoutd.Policy{{
		Asset:             "USDC",
		DailyCap:          big.NewInt(10_000_000_000),
		SoftInventory:     big.NewInt(10_000_000_000),
	...
	first := &swap.CashOutIntent{
		IntentID:     "intent-v1",
		Account:      "nhb1velocity",
		StableAsset:  swap.StableAsset("USDC"),
		StableAmount: big.NewInt(100_000_000),
		NhbAmount:    big.NewInt(100_000_000),
	}
	second := &swap.CashOutIntent{
		IntentID:     "intent-v2",
		Account:      "nhb1velocity",
		StableAsset:  swap.StableAsset("USDC"),
		StableAmount: big.NewInt(100_000_000),
	...
	err = processor.Process(context.Background(), payoutd.CashOutRequest{
		Intent:      second,
		Destination: "0xrecipient2",
	})
	require.ErrorIs(t, err, payoutd.ErrVelocityExceeded)
	require.Equal(t, 1, mock.transfers)
}
