...
func TestBaseRewardImmediateSettlementRecordsPartialRatio(t *testing.T) {
	...
	spenderAcc := mustAccount(t, manager, spender)
	if spenderAcc.BalanceZNHB.Cmp(expectedPayout) != 0 {
		t.Fatalf("expected payout %s, got %s", expectedPayout.String(), spenderAcc.BalanceZNHB.String())
	}
