...
checkpoints the share index, and increments the year-to-date accounting meter. The node emits two receipts: the canonical
...
| Attribute | Type | Description |
| --- | --- | --- |
| `addr` | `string` | Delegator address receiving the treasury-funded rewards. |
| `minted` | `string` | Amount of ZNHB paid to the delegator (wei). |
