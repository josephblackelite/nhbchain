...
| Prefix `gov/params` | `[]QueryRecord` | `QueryPrefix("gov", "params")` is currently implemented by `queryGovernancePrefix` (`core/query_router.go`), which only returns the single hardcoded `staking.minimumValidatorStake` key — every other governance parameter is not exposed through this endpoint. |
