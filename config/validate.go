...
func ValidateConfig(g Global) error {
	...
	if trimmed := strings.TrimSpace(g.Fees.TransferFreeTierSpendWei); trimmed != "" {
		...
		if err != nil {
			return fmt.Errorf("fees: invalid transfer_free_tier_spend_wei: %w", err)
		}
		if amount.Sign() < 0 {
			return fmt.Errorf("fees: transfer_free_tier_spend_wei must be >= 0")
		}
	}
	switch strings.ToLower(strings.TrimSpace(g.Fees.TransferFreeTierWindow)) {
	case "", "lifetime", "monthly":
	default:
		return fmt.Errorf("fees: transfer_free_tier_window must be one of lifetime or monthly")
	}
