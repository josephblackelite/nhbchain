...
func TestSeedGenesisNHBSupplyOnce_UnblocksRealRedemption(t *testing.T) {
	...
	// ProcessBlockLifecycle also runs ReconcileNHBMintSupplyDriftOnce
	// (2026-09-17, the separate mint-side supply-tracking gap) on this same
	// fresh, never-reconciled processor, so its drift amount lands here too.
	expected := new(big.Int).Sub(new(big.Int).Add(genesisNHBSupplyWei, nhbMintSupplyDriftWei), burnAmount)
...
func TestSeedGenesisNHBSupplyOnce_SameBlockOrderingMatchesCreateBlock(t *testing.T) {
	...
	// ProcessBlockLifecycle (and the early node.go-style call above) also
	// run ReconcileNHBMintSupplyDriftOnce (2026-09-17, the separate
	// mint-side supply-tracking gap) on this same fresh, never-reconciled
	// processor, so its drift amount lands here too.
	expected := new(big.Int).Sub(new(big.Int).Add(genesisNHBSupplyWei, nhbMintSupplyDriftWei), burnAmount)
	if supply.Cmp(expected) != 0 {
...
// TestReconcileNHBMintSupplyDriftOnce_AddsExactAmount is the direct
// analogue of TestSeedGenesisNHBSupplyOnce_AddsExactAmount for the separate
// 2026-09-17 fix: applyMintTransaction credited real NHB balances without
// ever calling AdjustTokenSupply, so the tracked counter understated real
// circulating NHB by the sum of every historical mint. Confirms the
// reconciliation adds exactly nhbMintSupplyDriftWei on top of whatever the
// counter already holds, and sets its own guard flag.
func TestReconcileNHBMintSupplyDriftOnce_AddsExactAmount(t *testing.T) {
	sp := newStakingStateProcessor(t)
	manager := nhbstate.NewManager(sp.Trie)

	priorSupply := big.NewInt(0)
	priorSupply.SetString("9980000000000000000000", 10) // 9,980 NHB, standing in for the live counter's real value
	seedTokenSupply(t, sp, priorSupply)
	...
	after, err := manager.TokenSupply("NHB")
	if err != nil {
		t.Fatalf("load supply after reconcile: %v", err)
	}
	expected := new(big.Int).Add(priorSupply, nhbMintSupplyDriftWei)
	if after.Cmp(expected) != 0 {
		t.Fatalf("expected supply %s (prior + drift), got %s", expected, after)
	}
	...
	if !reconciled {
		t.Fatalf("expected NHBMintSupplyDriftReconciled flag to be set after a successful reconcile")
	}
}

// TestReconcileNHBMintSupplyDriftOnce_IsIdempotent confirms a second call
// is a true no-op -- critical, since ProcessBlockLifecycle and all three
// core/node.go call sites call this unconditionally on every block forever.
func TestReconcileNHBMintSupplyDriftOnce_IsIdempotent(t *testing.T) {
	sp := newStakingStateProcessor(t)
	manager := nhbstate.NewManager(sp.Trie)

	if err := sp.ReconcileNHBMintSupplyDriftOnce(); err != nil {
		t.Fatalf("first reconcile call: %v", err)
	}
	firstSupply, err := manager.TokenSupply("NHB")
	if err != nil {
		t.Fatalf("load supply after first reconcile: %v", err)
	}

	if err := sp.ReconcileNHBMintSupplyDriftOnce(); err != nil {
		t.Fatalf("second reconcile call: %v", err)
	}
	secondSupply, err := manager.TokenSupply("NHB")
	if err != nil {
		t.Fatalf("load supply after second reconcile: %v", err)
	}
