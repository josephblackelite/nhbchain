...
func (n *Node) publishPOSFinality(update POSFinalityUpdate) {
	...
	if len(update.TxHash) > 0 && len(update.IntentRef) > 0 {
		key := hex.EncodeToString(update.IntentRef)
		if n.posIntentTxHash == nil {
			n.posIntentTxHash = make(map[string][]byte)
		}
		if _, exists := n.posIntentTxHash[key]; !exists {
			n.posIntentTxHashOrder = append(n.posIntentTxHashOrder, key)
		}
		n.posIntentTxHash[key] = append([]byte(nil), update.TxHash...)
		if len(n.posIntentTxHashOrder) > posFinalityHistoryLimit {
			excess := len(n.posIntentTxHashOrder) - posFinalityHistoryLimit
			for _, evictKey := range n.posIntentTxHashOrder[:excess] {
				delete(n.posIntentTxHash, evictKey)
			}
			remaining := make([]string, posFinalityHistoryLimit)
			copy(remaining, n.posIntentTxHashOrder[excess:])
			n.posIntentTxHashOrder = remaining
		}
	}
...
// PosIntentTxHash returns the transaction hash most recently associated
// with intentRef in this validator's in-memory POS finality cache. This is
// a best-effort, RPC-display-only lookup: capped at posFinalityHistoryLimit
// distinct intents, wiped on every restart, and independent per validator
// (each validator only ever sees intents/blocks it itself processed
// recently) -- a miss means "unknown to this validator right now," never
// "never existed." No reconciliation or settlement logic may treat this as
// authoritative.
func (n *Node) PosIntentTxHash(intentRef []byte) ([]byte, bool) {
	if n == nil || len(intentRef) == 0 {
		return nil, false
	}
	key := hex.EncodeToString(intentRef)
	n.posStreamMu.RLock()
	defer n.posStreamMu.RUnlock()
	hash, ok := n.posIntentTxHash[key]
	if !ok {
		return nil, false
	}
