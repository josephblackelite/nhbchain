...
// understated real circulating NHB more with every custody-deposit mint.
// Confirms a real, end-to-end mint (through the same CreateBlock/CommitBlock
// path TestMintWithSignatureExecutesInBlock exercises) increases the
// tracked token/supply/NHB counter by exactly the minted amount, mirroring
// applyRedeemNHB's already-correct AdjustTokenSupply call on the burn side.
func TestMintWithSignatureIncrementsTokenSupply(t *testing.T) {
	node := newTestNode(t)
	...
	// Commit one empty block first so the one-time genesis-seed and
	// mint-supply-drift reconciliations (which also fire on this fresh
	// node's very first block) have already settled -- isolating this
	// test to purely the mint's own contribution, rather than hardcoding
	// an assumption about what else runs alongside a node's first block.
	emptyBlock, err := node.CreateBlock(nil)
	if err != nil {
		t.Fatalf("create empty block: %v", err)
	}
	if err := node.CommitBlock(emptyBlock); err != nil {
		t.Fatalf("commit empty block: %v", err)
	}

	node.stateMu.Lock()
	before, err := nhbstate.NewManager(node.state.Trie).TokenSupply("NHB")
	node.stateMu.Unlock()
	if err != nil {
		t.Fatalf("load supply before mint: %v", err)
	}

	voucher := MintVoucher{
		InvoiceID: "inv-supply-tracking",
		Recipient: recipientKey.PubKey().Address().String(),
		Token:     "NHB",
		Amount:    "125",
		ChainID:   MintChainID,
	...
	// CommitBlock replaces node.state.Trie with a fresh handle on the newly
	// committed root -- the pre-mint manager above is now stale and must
	// not be reused, or this would silently read the old, pre-commit state.
	node.stateMu.Lock()
	after, err := nhbstate.NewManager(node.state.Trie).TokenSupply("NHB")
	node.stateMu.Unlock()
	if err != nil {
		t.Fatalf("load supply after mint: %v", err)
	}
	amount, _ := voucher.AmountBig()
	expected := new(big.Int).Add(before, amount)
	if after.Cmp(expected) != 0 {
		t.Fatalf("expected supply %s after mint (before + minted amount), got %s -- mint is not updating the tracked token supply", expected, after)
	}
}
