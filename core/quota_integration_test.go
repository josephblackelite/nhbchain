...
func TestQuotaEnforcementRedeemNHB(t *testing.T) {
	...
	// Scaled to whole NHB (2026-09-17) so both redemptions clear the new 5
	// NHB floor -- this test is about quota enforcement, not the floor.
	firstTx := redeemNHBTx(t, 0, nhbAmount(10), "usdttrc20", "T9yD14Nj9j7xAB4dbGeiX9h8unkKHxuWwb")
	...
	secondTx := redeemNHBTx(t, 1, nhbAmount(10), "usdttrc20", "T9yD14Nj9j7xAB4dbGeiX9h8unkKHxuWwb")
	acc, err = sp.getAccount(userAddr)
	if err != nil {
		t.Fatalf("get account: %v", err)
	}
