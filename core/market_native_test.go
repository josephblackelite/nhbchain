...
// setUpMarketAdminInvariantTest seeds an admin/treasury wallet (with a
// bootstrapped ZNHB pool ledger the invariant checks against) the same way
// core/transfer_znhb_test.go's admin-wallet tests do, so
// CheckZNHBSupplyInvariant is meaningful immediately.
func setUpMarketAdminInvariantTest(t *testing.T) (sp *StateProcessor, adminKey *crypto.PrivateKey, adminAddr [20]byte) {
	t.Helper()
	sp = newStakingStateProcessor(t)
	var err error
	...
	// BalanceNHB is wei-scaled (1000 NHB) -- comfortably enough for the fill
	// test's NHB cost plus flat fee, mirroring
	// TestApplyMarketFillListing_PartialThenFullSettlesCorrectly's buyerFunds
	// convention. BalanceZNHB stays a plain small unit, matching every other
	// ZNHB amount in this test file (including the admin-wallet seed in
	// core/transfer_znhb_test.go's equivalent tests).
	adminNHB := new(big.Int).Mul(big.NewInt(1000), big.NewInt(1_000000000000000000))
	...
	return sp, adminKey, adminAddr
}

// TestApplyMarketCreateListing_AdminSellerKeepsSupplyInvariantSatisfied
// reproduces the live 2026-09-16 chain halt (found by deliberately testing
// the admin wallet against the market, right after the ZNHB-transfer
// version of this same bug class was fixed): CreateListing moves the
// seller's ZNHB into marketEscrowAddr, a dedicated module address
// adminZNHBOwned never reads. When the seller is the admin/treasury wallet,
// that's ZNHB leaving its tracked balance with no offsetting Sale/Reward
// Pool adjustment -- CheckZNHBSupplyInvariant catches it on the very next
// block and halts the chain, exactly like the transfer-fee incidents this
// mirrors.
func TestApplyMarketCreateListing_AdminSellerKeepsSupplyInvariantSatisfied(t *testing.T) {
	sp, adminKey, _ := setUpMarketAdminInvariantTest(t)

	// 50,000 -- the live incident's reproduction quantity (ZNHB amounts in
	// this test file are plain small units, not wei-scaled; see
	// setUpMarketAdminInvariantTest's admin BalanceZNHB seed).
	listAmount := big.NewInt(50_000)
	tx := marketCreateListingTx(t, 0, listAmount, big.NewInt(1), big.NewInt(1), true)
	if err := tx.Sign(adminKey.PrivateKey); err != nil {
		t.Fatalf("sign create listing: %v", err)
	}
	if err := sp.ApplyTransaction(tx); err != nil {
		t.Fatalf("apply create listing: %v", err)
	...
	if err := sp.CheckZNHBSupplyInvariant(); err != nil {
		t.Fatalf("supply invariant violated after the admin wallet listed ZNHB for sale -- this is the exact 2026-09-16 chain-halt bug (market module): %v", err)
	}
}

// TestApplyMarketFillListing_AdminBuyerKeepsSupplyInvariantSatisfied covers
// the symmetric case: the admin wallet as BUYER receives ZNHB straight out
// of marketEscrowAddr via FillListing, genuinely new ZNHB landing on its
// tracked balance from a module address adminZNHBOwned never reads.
func TestApplyMarketFillListing_AdminBuyerKeepsSupplyInvariantSatisfied(t *testing.T) {
	sp, adminKey, adminAddr := setUpMarketAdminInvariantTest(t)
	...
	createTx := marketCreateListingTx(t, 0, big.NewInt(1_000), big.NewInt(1), big.NewInt(1), true)
	if err := createTx.Sign(sellerKey.PrivateKey); err != nil {
		t.Fatalf("sign create listing: %v", err)
	}
	if err := sp.ApplyTransaction(createTx); err != nil {
	...
	if err := sp.CheckZNHBSupplyInvariant(); err != nil {
		t.Fatalf("invariant should still hold after an ordinary (non-admin) listing: %v", err)
	}
	listingID := soleOpenListingID(t, sp)

	fillTx := marketFillListingTx(t, 0, listingID, big.NewInt(1_000))
	if err := fillTx.Sign(adminKey.PrivateKey); err != nil {
		t.Fatalf("sign fill listing: %v", err)
	}
	if err := sp.ApplyTransaction(fillTx); err != nil {
		t.Fatalf("apply fill listing (admin as buyer): %v", err)
	}
	...
	if admin.BalanceZNHB.Cmp(new(big.Int).Add(big.NewInt(996_987_257), big.NewInt(1_000))) != 0 {
		t.Fatalf("expected admin wallet to receive the filled ZNHB, got %s", admin.BalanceZNHB)
	}
	if err := sp.CheckZNHBSupplyInvariant(); err != nil {
		t.Fatalf("supply invariant violated after the admin wallet bought ZNHB off the market -- this is the market-module fill-side counterpart of the 2026-09-16 chain-halt bug: %v", err)
	}
}

// TestApplyMarketCancelListing_AdminSellerKeepsSupplyInvariantSatisfied
// covers the reversal: CancelListing refunds the listing's unsold ZNHB from
// marketEscrowAddr back to the seller, the exact reverse of CreateListing's
// debit. When the seller is the admin/treasury wallet, that reversal must
// be mirrored back into the pool too, or the pool stays permanently short
// by the refunded amount even though CreateListing's own debit was
// correctly offset.
func TestApplyMarketCancelListing_AdminSellerKeepsSupplyInvariantSatisfied(t *testing.T) {
	sp, adminKey, adminAddr := setUpMarketAdminInvariantTest(t)

	listAmount := big.NewInt(2_500)
	createTx := marketCreateListingTx(t, 0, listAmount, big.NewInt(1), big.NewInt(1), true)
	if err := createTx.Sign(adminKey.PrivateKey); err != nil {
		t.Fatalf("sign create listing: %v", err)
	}
	if err := sp.ApplyTransaction(createTx); err != nil {
		t.Fatalf("apply create listing: %v", err)
	...
	if err := sp.CheckZNHBSupplyInvariant(); err != nil {
		t.Fatalf("invariant should hold right after the admin wallet's own listing is created: %v", err)
	}
	listingID := soleOpenListingID(t, sp)

	cancelTx := marketCancelListingTx(t, 1, listingID)
	if err := cancelTx.Sign(adminKey.PrivateKey); err != nil {
		t.Fatalf("sign cancel listing: %v", err)
	}
	if err := sp.ApplyTransaction(cancelTx); err != nil {
		t.Fatalf("apply cancel listing (admin as seller): %v", err)
	}
	...
	if admin.BalanceZNHB.Cmp(big.NewInt(996_987_257)) != 0 {
		t.Fatalf("expected admin wallet fully refunded after cancel, got %s", admin.BalanceZNHB)
	}
	if err := sp.CheckZNHBSupplyInvariant(); err != nil {
		t.Fatalf("supply invariant violated after the admin wallet cancelled its own listing: %v", err)
	}
}
