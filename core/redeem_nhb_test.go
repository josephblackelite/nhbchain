...
// nhbAmount converts a whole-NHB count into its wei (18-decimal) value.
// Added 2026-09-17 alongside the new 5 NHB (5e18 wei) redemption floor
// (see native/swap/redeem_risk.go's DefaultRedeemPerTxMinWei): this file's
// tests used to burn/seed plain small integers like 400 (meaning 400 WEI,
// i.e. 0.0000000000000004 NHB) purely for arithmetic convenience, which
// the new floor now rejects outright. Every prior literal N here becomes
// nhbAmount(N) instead, preserving each test's exact relative arithmetic
// (1000 seeded - 400 burned = 600 remaining, etc.) at a realistic,
// floor-clearing scale.
func nhbAmount(whole int64) *big.Int {
...
func TestApplyRedeemNHB_BurnsBalanceAndRecordsPendingRequest(t *testing.T) {
	...
	if user.BalanceNHB.Cmp(nhbAmount(600)) != 0 {
	...
	if request.NHBAmountWei != nhbAmount(400).String() {
...
func TestApplyRedeemNHB_InsufficientBalanceRejected(t *testing.T) {
	...
	// Both sides scaled to whole NHB (2026-09-17) so this rejects for
	// insufficient balance specifically, not for falling below the new 5
	// NHB redemption floor -- 400 still exceeds 100, same as before.
	tx := redeemNHBTx(t, 0, nhbAmount(400), "usdttrc20", "T9yD14Nj9j7xAB4dbGeiX9h8unkKHxuWwb")
	if err := tx.Sign(userKey.PrivateKey); err != nil {
		t.Fatalf("sign transaction: %v", err)
	}
	...
	if user.BalanceNHB.Cmp(nhbAmount(100)) != 0 {
...
func TestApplyRedeemNHB_RequiresDestination(t *testing.T) {
	...
	// Scaled to whole NHB (2026-09-17) so this rejects for the missing
	// destination specifically, not for falling below the new 5 NHB floor.
	tx := redeemNHBTx(t, 0, nhbAmount(400), "", "")
	if err := tx.Sign(userKey.PrivateKey); err != nil {
		t.Fatalf("sign transaction: %v", err)
	}
	if err := sp.ApplyTransaction(tx); err == nil {
...
func TestApplyAttestRedemption_RejectsUnauthorizedSigner(t *testing.T) {
	...
	userAddr := userKey.PubKey().Address().Bytes()
	if err := sp.setAccount(userAddr, &types.Account{BalanceNHB: nhbAmount(1_000), BalanceZNHB: big.NewInt(0), Stake: big.NewInt(0)}); err != nil {
		t.Fatalf("seed user: %v", err)
	}
	seedTokenSupply(t, sp, nhbAmount(1_000))
	burnTx := redeemNHBTx(t, 0, nhbAmount(400), "usdttrc20", "dest")
	...
	requestID := nhbstate.RedemptionRequestID(userAddr, burnHash)
...
func TestApplyAttestRedemption_MarksFailedAndRefundsBurn(t *testing.T) {
	...
	if supply.Cmp(nhbAmount(1_000)) != 0 {
	...
	for _, evt := range sp.Events() {
		if evt.Type == events.TypeRedemptionRefunded {
			sawRefundEvent = true
			if evt.Attributes["requestId"] != requestID || evt.Attributes["nhbAmount"] != nhbAmount(400).String() {
...
func TestApplyAttestRedemption_CannotDoubleRefund(t *testing.T) {
	...
	spendTx := redeemNHBTx(t, 1, nhbAmount(300), "usdttrc20", "dest2")
	...
	if user.BalanceNHB.Cmp(nhbAmount(700)) != 0 {
...
func TestApplyRedeemNHB_DecrementsTokenSupply(t *testing.T) {
	...
	if before.Cmp(nhbAmount(1_000)) != 0 {
...
// the index (what an off-chain watcher will poll), and attesting it paid
// must remove it again so the index doesn't grow unbounded.
...
// still be attestable -- an off-chain attestor needs to be able to close
// out in-flight payouts even while new burns are frozen for an incident.
func TestApplyRedeemNHB_PauseGuardBlocksNewBurnsButNotAttestation(t *testing.T) {
	...
	firstBurn := redeemNHBTx(t, 0, nhbAmount(400), "usdttrc20", "dest")
	...
	requestID := nhbstate.RedemptionRequestID(userAddr, firstBurnHash)

	// Now pause new redemptions.
	sp.SetPauseView(pauseViewStub{modules: map[string]bool{moduleSwapRedeem: true}})

	// Scaled to whole NHB (2026-09-17) so this rejects for the pause
	// specifically, not for falling below the new 5 NHB floor.
	secondBurn := redeemNHBTx(t, 1, nhbAmount(100), "usdttrc20", "dest")
...
func TestApplyRedeemNHB_RejectsAboveConfiguredPerTxMax(t *testing.T) {
	...
	// This test deliberately exercises PerTxMaxWei at a tiny artificial
	// scale (300 wei), unrelated to the new 5 NHB (5e18 wei) PerTxMinWei
	// default (2026-09-17) -- override the floor to 0 so the amount below
	// is rejected for exceeding the configured max, not for falling below
	// an unrelated floor this test was never written to test.
	if err := manager.ParamStoreSet(governance.ParamKeySwapRiskRedeemPerTxMinWei, []byte("0")); err != nil {
		t.Fatalf("seed redeem per-tx min: %v", err)
	}
...
func TestApplyRedeemNHB_RejectsAboveConfiguredDailyCap(t *testing.T) {
	...
	// This test deliberately exercises the daily cap at a tiny artificial
	// scale (300/500 wei), unrelated to the new 5 NHB (5e18 wei)
	// PerTxMinWei default (2026-09-17) -- override the floor to 0 so both
	// redemptions below are governed only by the max/daily-cap params this
