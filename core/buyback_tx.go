...
// ErrBuybackRefPriceAlreadyRecorded indicates a reference price for the
// submitted epoch has already been recorded on-chain. Once true for a given
// epoch this can never become false again -- the epoch number only moves
// forward -- so a transaction hitting this is a permanently dead duplicate,
// exactly like ErrRedeemRequestExists/ErrMintInvoiceUsed, not a transient
// condition worth retrying. Before this sentinel existed, this error was a
// bare fmt.Errorf that classifyProposalError (core/node.go) could not
// recognize, so it fell through to proposalDispositionAbort -- aborting the
// ENTIRE block-build attempt, forever, on every retry, any time a stray
// duplicate ref-price submission (e.g. a resubmission race from the
// off-chain reference-price submitter) sat in the mempool alongside other
// unrelated transactions. This is the
// production incident the 2026-08-13 "buyback ref-price tx halts consensus"
// investigation (task #138) never found: it only ever tested an isolated
// single-buyback-tx block, never a mixed block, which is exactly the case
...
func (sp *StateProcessor) applyBuybackRefPrice(tx *types.Transaction) error {
	...
	if _, exists, err := manager.BuybackRefPriceForEpoch(epochNumber); err != nil {
		...
		return fmt.Errorf("%w (epoch %d)", ErrBuybackRefPriceAlreadyRecorded, epochNumber)
	}
