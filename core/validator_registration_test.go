...
func TestValidatorVotingPower_IncludesDelegatedIn(t *testing.T) {
	...
	// By product design, a third party delegating ZNHB to this validator IS
	// this validator's own stake for eligibility purposes (see
	// stakeRewardBasis's doc comment and docs/validators/onboarding.md's
	// "anyone can become a validator" delegation flow) -- there is no
	// separate self-stake requirement. The combined total, not just the
	// original self-stake, must count.
	basisAfter, ok := sp.EligibleValidators[string(validator[:])]
	if !ok {
		t.Fatalf("expected validator to remain eligible after delegation")
	}
	if basisAfter.Cmp(big.NewInt(46_000)) != 0 {
		t.Fatalf("eligibility basis must equal the combined total (46000) after delegation: got %s", basisAfter)
	}
	...
	// sums via core/node.go's GetValidatorSet -- is also the combined
	// total, not just the original self-stake.
	...
	if votingPower.Cmp(big.NewInt(46_000)) != 0 {
		t.Fatalf("BFT voting power must equal the combined total 46000: got %s", votingPower)
	}
