...
var (
	...
	znhbValidatorFundingDriftKey     = []byte("znhb/adminValidatorFundingDriftReconciled")
	...
	nhbMintSupplyDriftReconciledKey  = []byte("nhb/supply/mintDriftReconciled")
...
func (m *Manager) StakingGlobalIndex() (*big.Int, error) {
	...
	snapshot, snapErr := m.GetGlobalIndex()
	if snapErr != nil {
		return nil, err
	}
	if snapshot == nil {
	...
	return decodeUQ128x128(snapshot.UQ128x128), nil
}
...
// StakingEmissionYTD fetches the cumulative staking rewards accounted for within the
...
// SetStakingEmissionYTD overwrites the recorded year-to-date staking reward
...
// ZNHBAdminValidatorFundingDriftReconciled reports whether the one-time
// repair for the 2026-09-09 incident (see
// StateProcessor.ReconcileAdminValidatorFundingDriftOnce's doc comment) has
// already run.
func (m *Manager) ZNHBAdminValidatorFundingDriftReconciled() (bool, error) {
	var flag bool
	ok, err := m.KVGet(znhbValidatorFundingDriftKey, &flag)
	if err != nil {
		return false, err
	}
	return ok && flag, nil
...
// ZNHBMarkAdminValidatorFundingDriftReconciled records that the one-time
// 2026-09-09 drift repair has run, so it is never repeated.
func (m *Manager) ZNHBMarkAdminValidatorFundingDriftReconciled() error {
	return m.KVPut(znhbValidatorFundingDriftKey, true)
}
...
// NHBMintSupplyDriftReconciled reports whether the one-time repair of
// applyMintTransaction's missing AdjustTokenSupply call (see
// StateProcessor.ReconcileNHBMintSupplyDriftOnce in
// core/state_transition.go) has already run.
func (m *Manager) NHBMintSupplyDriftReconciled() (bool, error) {
	var flag bool
	ok, err := m.KVGet(nhbMintSupplyDriftReconciledKey, &flag)
	if err != nil {
		return false, err
	}
	return ok && flag, nil
...
// MarkNHBMintSupplyDriftReconciled records that the one-time mint-supply
// drift repair has run, so it is never repeated -- any future discrepancy
// after this flag is set is a real new bug that must be investigated, not
// papered over again.
func (m *Manager) MarkNHBMintSupplyDriftReconciled() error {
	return m.KVPut(nhbMintSupplyDriftReconciledKey, true)
}
