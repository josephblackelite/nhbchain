...
// removeStakeOwner pattern. This is how an off-chain watcher discovers
// pending swap-out requests without scanning the whole state trie. Entries
// are appended on creation (PutRedemptionRequest) and removed on
// finalisation (UpdateRedemptionStatus) -- the removal is not optional:
// KVAppend's underlying storage is an O(n) full-rewrite per append (see
// manager.go's KVAppend), so an index that only ever grows would make every
// future redemption progressively more expensive to record.
...
// the burn sender's address AND the burn transaction's hash, hex-encoded.
//
// txHash alone is NOT sufficient: core/types/transaction.go's V3 tx hash
// (sha256 over ChainID/Type/Nonce/MaxBlockHeight/IntentExpiry/IntentRef/To/
// Value/Data/GasLimit/GasPrice/Paymaster/MerchantAddress/DeviceID/RefundOf)
// never includes the sender or signature. TxTypeRedeemNHB always sets To to
// the zero address and uses constant gas defaults, so two DIFFERENT accounts
// that happen to submit a redeem with the same nonce (0, for anyone's
// first-ever transaction), amount, and destination payload produce the
// IDENTICAL tx.Hash() -- confirmed live 2026-09-17: a brand-new account's
// very first redemption ($10/10 NHB) collided with an unrelated, already-
// settled request from a different account, permanently blocking that exact
// (account, nonce, amount, destination) combination with no burn ever having
// happened. Folding the sender into this ID's own hash input, rather than
// changing tx.Hash() itself (used pervasively far beyond redemptions),
// scopes uniqueness to what actually needs to be unique -- one burn per
// account -- without touching transaction hashing consensus-wide.
func RedemptionRequestID(sender []byte, txHash []byte) string {
	combined := make([]byte, 0, len(sender)+len(txHash))
	combined = append(combined, sender...)
	combined = append(combined, txHash...)
	return hex.EncodeToString(ethcrypto.Keccak256(combined))
}
