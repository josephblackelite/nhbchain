...
// TestApplyTransferZNHB_AdminWalletSendingAsOwnFeeCollectorKeepsSupplyInvariantSatisfied
// reproduces the live 2026-09-16 chain halt: the admin/treasury wallet is
// FeeCollector by default (see buildTransferGasPolicyFromConfig), so when the
// admin wallet itself SENDS a ZNHB transfer, it pays its own gas fee to
// itself. That fee gets refunded back onto the same wallet (net-zero real
// balance effect from the fee), but the pre-fix code still routed the
// "credit" into the Reward Pool as if it were genuine external revenue,
// double-counting against the separate sender-is-admin debit that already
// subtracts the full transferred amount from the pool to mirror the real
// balance decrease. The pool ended up exactly one transfer's gasCost ahead
// of the admin wallet's real balance, halting block production on the very
// next block. Recipient is an ordinary third party here (not the admin
// wallet) -- the bug reproduces for any recipient, not just self-transfers.
func TestApplyTransferZNHB_AdminWalletSendingAsOwnFeeCollectorKeepsSupplyInvariantSatisfied(t *testing.T) {
	sp := newStakingStateProcessor(t)
	...
	// FeeCollector defaults to the admin/treasury wallet in production (see
	// buildTransferGasPolicyFromConfig) -- set explicitly here to match.
	sp.SetTransferGasPolicy(TransferGasPolicy{
		Enabled:      false, // force the fee regardless of free-tier bookkeeping
		FeeCollector: adminAddr,
		FeeBps:       9_999,
	...
	if err := sp.CheckZNHBSupplyInvariant(); err != nil {
		t.Fatalf("supply invariant violated after the admin wallet sent ZNHB while acting as its own fee collector -- this is the exact 2026-09-16 chain-halt bug: %v", err)
	}
}
