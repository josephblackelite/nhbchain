...
func (sp *StateProcessor) ProcessBlockLifecycle(height uint64, timestamp int64) error {
	...
	// One-time repair for the separate mint-side supply-tracking gap (see
	// ReconcileNHBMintSupplyDriftOnce's doc comment). No underflow risk
	// like the genesis seed above (this only ever adds), so ordering
	// relative to it doesn't matter -- grouped here for the same reason.
	...
	// One-time repair for the 2026-09-09 incident: the admin wallet sent
	// 10,000 ZNHB out via an ordinary transfer to fund a new wallet's own
	// validator self-stake (see
	// ReconcileAdminValidatorFundingDriftOnce's doc comment) -- must run
	// before the invariant check below, and is a no-op forever after its
	// guard flag is set, so it can never mask a genuine future violation.
	if err := sp.ReconcileAdminValidatorFundingDriftOnce(); err != nil {
		return fmt.Errorf("reconcile admin validator funding drift: %w", err)
	}
...
func (sp *StateProcessor) computeEpochWeights(now time.Time) ([]epoch.Weight, *big.Int, error) {
	...
	for addrKey := range sp.EligibleValidators {
		...
		basis, err := sp.validatorEligibilityBasis(account, addrBytes)
		if err != nil {
			return nil, nil, err
		}
