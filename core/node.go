...
type Node struct {
	...
	// posIntentTxHash/posIntentTxHashOrder together form a small
	// best-effort FIFO cache from hex(IntentRef) to the last-seen tx hash
	// for that intent (see core/pos_stream.go's publishPOSFinality /
	// PosIntentTxHash). Kept as its own ordered-key structure rather than
	// derived from posStreamHistory's own slice-position eviction, since
	// each intent produces two history entries (pending, then finalized)
	// sharing one IntentRef -- trimming on the first entry's age would
	// evict a still-live cache entry while the second is still present.
	posIntentTxHash      map[string][]byte
	posIntentTxHashOrder []string
}
...
func NewNode(db storage.Database, key *crypto.PrivateKey, genesisPath string, allowAutogenesis bool, allowMigrate bool) (*Node, error) {
	...
	node := &Node{
		...
		posIntentTxHash:  make(map[string][]byte),
	}
...
func (n *Node) validateTransaction(tx *types.Transaction) error {
	...
	// Admission-time simulation must see the same on-chain pause state that
	// real block-building sees (buildProposalState's refreshModulePauses +
	// SetPauseView, in CreateBlock above). Without this, a stateCopy here
	// carries a nil/stale pause view, so a paused-module transaction can
	// pass admission (get a real tx hash back to the caller) and then be
	// silently SKIP-classified forever at real execution -- permanently
	// reserving the sender's nonce with no error ever surfaced anywhere.
	if err := n.refreshModulePauses(); err != nil {
		return err
	}
...
func (n *Node) ValidateBlock(b *types.Block) error {
	...
	if err := stateCopy.ReconcileNHBMintSupplyDriftOnce(); err != nil {
		return fmt.Errorf("reconcile NHB mint supply drift: %w", err)
	}
...
// PotsoWeight returns the exact voting-power basis-points figure
// native/governance/engine.go's CastVote would compute for addr at the
// given epoch (0 = last processed epoch), by calling the SAME
// SnapshotPotsoWeights lookup CastVote itself uses -- deliberately NOT
// PotsoMetricsGetSnapshot (PotsoLeaderboard's key, above). See
// SetSnapshotPotsoWeights's own doc comment (core/state/manager.go): the
// two keys are allowed to diverge later even though today's writer
// populates them identically. Unlike CastVote, a missing snapshot or an
// address with no recorded entry both return a zero weight, not an error --
// this is a passive eligibility check, not a vote being cast.
func (n *Node) PotsoWeight(epoch uint64, addr [20]byte) (uint64, uint64, error) {
	n.stateMu.Lock()
	defer n.stateMu.Unlock()
	...
	snapshot, ok, err := manager.SnapshotPotsoWeights(target)
	if err != nil {
		return target, 0, err
	}
	if !ok || snapshot == nil {
	...
	for _, entry := range snapshot.Entries {
		if entry.Address == addr {
			return target, entry.WeightBps, nil
		}
	}
	return target, 0, nil
}
...
// MintWithSignature's contract; callers must poll swap_voucher_get (or this
// method's ledger record) until the voucher transitions to Minted. Note that
// swap_submitVoucher decodes into swap.VoucherV1 (domain/orderId/nonce
// fields required), a different wire shape from the unrelated mint_with_sig
// RPC's core.MintVoucher -- callers must not conflate the two payload
// formats.
//
...
// is how an off-chain watcher discovers burns awaiting an off-chain
// stablecoin payout -- see core/state/redemption.go's pending-request
