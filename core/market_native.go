...
func (sp *StateProcessor) applyMarketCreateListing(tx *types.Transaction, sender []byte) error {
	...
	// CreateListing moves tx.Value out of the seller's BalanceZNHB into
	// marketEscrowAddr -- a dedicated module address adminZNHBOwned never
	// reads. When the seller is the admin/treasury wallet, that's ZNHB
	// leaving its tracked balance with no offsetting pool change; mirror it
	// here or CheckZNHBSupplyInvariant halts the chain on the very next
	// block (see adjustRewardPoolForAdminZNHBMovement's doc comment).
	if sp.hasAdminWallet && bytes.Equal(sender, sp.adminWallet[:]) {
		if err := sp.adjustRewardPoolForAdminZNHBMovement(new(big.Int).Neg(tx.Value)); err != nil {
			return fmt.Errorf("marketCreateListing: %w", err)
		}
	}
...
func (sp *StateProcessor) applyMarketFillListing(tx *types.Transaction, sender []byte) error {
	...
	// FillListing pays the buyer payload.ZNHBAmount out of marketEscrowAddr
	// straight into their BalanceZNHB. When the buyer is the admin/treasury
	// wallet, that's genuinely new ZNHB landing on its tracked balance from
	// a module address adminZNHBOwned never reads -- mirror it into the
	// pool or CheckZNHBSupplyInvariant halts the chain on the very next
	// block (see adjustRewardPoolForAdminZNHBMovement's doc comment). The
	// seller only ever receives NHB here (ZNHB already left their tracked
	// balance back at CreateListing time), so no seller-side case exists.
	if sp.hasAdminWallet && bytes.Equal(sender, sp.adminWallet[:]) {
		if err := sp.adjustRewardPoolForAdminZNHBMovement(new(big.Int).Set(payload.ZNHBAmount)); err != nil {
			return fmt.Errorf("marketFillListing: %w", err)
		}
	}
	return sp.incrementNativeAccountNonce(sender)
...
func (sp *StateProcessor) applyMarketCancelListing(tx *types.Transaction, sender []byte) error {
	...
	seller := crypto.MustNewAddress(crypto.NHBPrefix, append([]byte(nil), sender...))
	// Captured before CancelListing so the admin-wallet pool adjustment
	// below (if any) uses the exact amount actually refunded -- CancelListing
	// itself returns only an error, and by the time it succeeds the
	// listing's RemainingAmount field is what it just credited back to the
	// seller (see native/market/engine.go's CancelListing).
	var refundAmount *big.Int
	if sp.hasAdminWallet && bytes.Equal(sender, sp.adminWallet[:]) {
		listing, err := sp.marketStateAdapter().GetListing(listingID)
		if err != nil {
			return fmt.Errorf("marketCancelListing: load listing for admin-wallet adjustment: %w", err)
		}
		if listing != nil {
			refundAmount = new(big.Int).Set(listing.RemainingAmount)
		}
	}
	if err := sp.marketEngine().CancelListing(seller, listingID); err != nil {
		return err
	}
	// CancelListing returns the listing's unsold ZNHB from marketEscrowAddr
	// back into the seller's BalanceZNHB -- the exact reverse of
	// CreateListing's debit. When the seller is the admin/treasury wallet,
	// mirror that reversal into the pool too (see
	// adjustRewardPoolForAdminZNHBMovement's doc comment) or the pool stays
	// short by the refunded amount.
	if refundAmount != nil {
		if err := sp.adjustRewardPoolForAdminZNHBMovement(refundAmount); err != nil {
			return fmt.Errorf("marketCancelListing: %w", err)
		}
	}
	return sp.incrementNativeAccountNonce(sender)
