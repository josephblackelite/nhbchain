...
type RPCNodeClient struct {
	...
	// unlike a single-writer-by-design off-chain watcher, escrow-gateway's
	// mutating endpoints can be hit by multiple concurrent HTTP requests,
	// and a blockchain account's nonce sequence is inherently sequential,
	// so submission itself must be serialized regardless of how many
