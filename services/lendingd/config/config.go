...
type Config struct {
	...
	NodeRPCToken       string     `yaml:"node_rpc_token"`
	SharedSecretHeader string     `yaml:"shared_secret_header"`
	SharedSecretValue  string     `yaml:"shared_secret_value"`
...
func Load(path string) (Config, error) {
	cfg := Config{
		ListenAddress:      ":50053",
		NodeRPCURL:         "https://127.0.0.1:8081",
		SharedSecretHeader: "X-NHB-Shared-Secret",
		RateLimitPerMin:    120,
	}
	if path == "" {
...
func (cfg *Config) normalize() {
	...
	cfg.NodeRPCURL = strings.TrimSpace(cfg.NodeRPCURL)
	if cfg.NodeRPCURL == "" {
		cfg.NodeRPCURL = "https://127.0.0.1:8081"
	}
	cfg.NodeRPCToken = strings.TrimSpace(cfg.NodeRPCToken)
	cfg.SharedSecretHeader = strings.TrimSpace(cfg.SharedSecretHeader)
	if cfg.SharedSecretHeader == "" {
		cfg.SharedSecretHeader = "X-NHB-Shared-Secret"
	}
	cfg.SharedSecretValue = strings.TrimSpace(cfg.SharedSecretValue)
...
func (cfg *Config) validate() error {
	...
	if strings.TrimSpace(cfg.NodeRPCURL) == "" {
